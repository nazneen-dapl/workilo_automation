...
for (def row = 1; row <= findTestData('signup_customer').getRowNumbers(); row++) {
  ...
  // try {
	 ...
	 //WebUI.verifyElementPresent(findTestObject('Object Repository/Sign up for service provider/Page_Workilo  Welcome/Duplicate Email'), 10)
	 //WebUI.click(findTestObject('Object Repository/Sign up for service provider/Page_Workilo  Welcome/Duplicate Email'))
	   //	}
  // catch (Exception e) {
	   //WebUI.verifyElementPresent(findTestObject('Object Repository/Sign up for service provider/Page_Workilo  Welcome/ShopCreatePage') , 10)
	 //  title = WebUI.getWindowTitle()
	  // WebUI.verifyMatch(title, "        Workilo :: Shop Setup", true)
	
	   WebUI.verifyElementPresent( findTestObject('Object Repository/Sign up for service provider/Page_Workilo  Shop Setup/Profile Image'), 10)
	...
	WebUI.setText(findTestObject('sign up for service provider recording/Page_Workilo  Shop Setup/input_user_zip'), findTestData(
			'SignUp_ServiceProvider').getValue('zip_code', row))

	WebUI.setText(findTestObject('sign up for service provider recording/Page_Workilo  Shop Setup/input_city'), findTestData(
	...
	*/
	...
	def productExist= WebUI.verifyElementPresent(findTestObject('Object Repository/Sign up for service provider/Page_Workilo  Listings/duplicate listing'), 10)
	if (productExist== true) {
				 println('duplicate service name')
	} else {
